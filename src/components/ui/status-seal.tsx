...
function SealMark({ reduced }: { reduced: boolean | null }) {
  return (
    <span className="relative flex size-11 shrink-0 items-center justify-center rounded-xl bg-forest-foreground/10 ring-1 ring-forest-foreground/15">
      {!reduced ? (
        <span className="absolute inset-0 rounded-xl ring-1 ring-cta/25" aria-hidden />
      ) : null}
      <ShieldCheckIcon className="relative size-5 text-cta" strokeWidth={2.25} />
      {!reduced ? (
        <span
          aria-hidden
...
export function StatusSeal({
  ...
  return (
    <div
      className={cn(
        "relative w-full overflow-hidden rounded-2xl bg-linear-to-br from-forest via-forest to-forest/92 px-4 py-3.5 text-forest-foreground sm:px-5 sm:py-4",
    ...
    >
      ...
      <div className="relative flex items-center gap-3.5 sm:gap-4">
        ...
        <div className="min-w-0 flex-1">
          ...
          <p className="mt-0.5 font-heading text-base font-semibold leading-tight sm:text-[1.05rem]">
            {title}
          </p>
        </div>
        <span className="inline-flex shrink-0 items-center gap-1.5 rounded-full bg-forest-foreground/10 px-2.5 py-1 text-xs font-semibold tracking-wide text-forest-foreground/90 ring-1 ring-forest-foreground/12">
          {!reduce ? (
            <span
              aria-hidden
